...
//!
//! Two impls:
//!   - `ThumbDvBackend`: byte-for-byte serial relay to a real
//!     DVSI AMBE-3000R chip.  Doesn't parse; the chip is the source
//!     of truth for protocol semantics.
//!   - `SoftBackend`: in-process software vocoder.  Parses each
//!     packet, fabricates the chip's responses for control packets,
//!     and runs encode/decode through `ambe::Vocoder`.  Refuses any
//!     non-DMR RATEP and poisons the session until the next RESET so
//!     a misconfigured client can't keep feeding data the vocoder
//!     would mis-decode.
...
#[cfg(feature = "thumbdv")]
pub(crate) use thumbdv::ThumbDvBackend;
#[cfg(feature = "thumbdv")]
mod thumbdv {
    use std::io::Read;
    use std::io::Write;
...
pub(crate) use soft::SoftBackend;
#[cfg(any(feature = "dynarmic", feature = "neural"))]
mod soft {
    use ambe::Vocoder;
    use anyhow::Result;
